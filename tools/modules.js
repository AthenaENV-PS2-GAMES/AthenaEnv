...
`;
    ...
    const libs = new Set();
    // IOP modules embedded on behalf of a module: name -> { irx, build }
    const irxs = new Map();
    ...
    for (const m of configuredModules) {
        ...
        for (const irx of m.embed_irx || []) {
            if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(irx.name || '') || !irx.irx) {
                throw new Error(`Module ${m.id}: embed_irx entries need a C identifier "name" and an "irx" path`);
            }
            const previous = irxs.get(irx.name);
            if (previous && previous.irx !== irx.irx) {
                throw new Error(`Module ${m.id}: IRX "${irx.name}" is already embedded from ${previous.irx}`);
            }
            irxs.set(irx.name, { irx: irx.irx, build: irx.build || null });
        }
    }

    let irxVars = '';
    for (const [name, irx] of irxs) {
        irxVars += `MODULE_IRX_PATH_${name} = ${irx.irx}\n`;
        if (irx.build) irxVars += `MODULE_IRX_BUILD_${name} = ${irx.build}\n`;
    }
...
# IOP modules embedded as <name>_irx / size_<name>_irx. Rules live in Makefile.embed.
MODULE_IRX =${irxs.size ? ` ${Array.from(irxs.keys()).join(' ')}` : ''}
${irxVars}`;
