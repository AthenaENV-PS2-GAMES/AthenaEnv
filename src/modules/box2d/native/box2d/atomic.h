...
// AthenaEnv: the R5900 has no LL/SC, so GCC lowers __atomic_* to libatomic
// calls the PS2SDK does not provide. Worlds run with a single worker
// (workerCount is forced to 1), so plain loads and stores are enough.
