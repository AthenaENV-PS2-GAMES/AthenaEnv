...
// AthenaEnv: the PS2 build never defines NDEBUG, so on the EE assertions
// follow the build type instead (DEBUG=1). The rule lives here so every file
// including Box2D agrees on it. Elsewhere the upstream rule applies.
#if defined( PS2 ) || defined( _EE )
#if defined( DEBUG ) || defined( B2_ENABLE_ASSERT )
#define B2_ASSERT_ENABLED 1
#endif
#elif !defined( NDEBUG ) || defined( B2_ENABLE_ASSERT )
#define B2_ASSERT_ENABLED 1
#endif

#ifdef B2_ASSERT_ENABLED
