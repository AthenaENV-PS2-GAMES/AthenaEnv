...
// AthenaEnv: the EE cycle counter (COP0 Count, 294.912 MHz). It wraps every
// ~14.5 s, much longer than anything Box2D times, so deltas are taken
// modulo 2^32. Feeds b2World_GetProfile().
#define B2_EE_TICKS_PER_MS 294912.0f

static inline uint32_t b2ReadCycleCounter( void )
{
	uint32_t count;
...
uint64_t b2GetTicks( void )
{
	return b2ReadCycleCounter();
}

float b2GetMilliseconds( uint64_t ticks )
{
	return (float)( b2ReadCycleCounter() - (uint32_t)ticks ) / B2_EE_TICKS_PER_MS;
}

float b2GetMillisecondsAndReset( uint64_t* ticks )
{
	uint32_t now = b2ReadCycleCounter();
	float ms = (float)( now - (uint32_t)*ticks ) / B2_EE_TICKS_PER_MS;
	*ticks = now;
	return ms;
}
