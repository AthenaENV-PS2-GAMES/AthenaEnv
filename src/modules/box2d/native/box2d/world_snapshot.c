...
static bool b2DeserializeIntoShell( b2SnapReader* r, b2World* world )
{
	...
	// Step 5: chain shapes
	{
		...
		// AthenaEnv: skipped for 0 chains, where data is NULL (memset(NULL, 0, 0) is undefined)
		if ( r->ok && chainCount > 0 )
		{
	...
	// Step 6: sensors
	{
		...
		// AthenaEnv: skipped for 0 elements (memset(NULL, 0, 0) is undefined)
...
// AthenaEnv: the header checks b2World_Restore runs before touching the world,
// so a caller can reject an image while the world is still intact.
bool b2IsValidSnapshotImage( const uint8_t* image, int size )
{
	b2SnapReader reader;
	return b2OpenSnapshotImage( image, size, &reader );
}
