...
// AthenaEnv: the arguments go through an unevaluated call instead of a comma
// expression, which GCC reports as -Wunused-value on every use.
static inline void b2UnusedArgs( int dummy, ... )
{
	(void)dummy;
}
#define B2_UNUSED( ... ) ( (void)sizeof( ( b2UnusedArgs( 0, __VA_ARGS__ ), 0 ) ) )
