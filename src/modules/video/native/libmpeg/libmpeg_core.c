...
{
    /*
     * End of data is sticky. Injecting the end code again on every call
     * restarts the bit window, so a byte-by-byte start code search
     * (_MPEG_NextStartCode) consumes 8 bits of it each time and never sees
     * 0x000001: an endless loop on streams that do not end in 0x000001B7.
     * The next refill (_ipu_sync_data) returns the end code aligned.
     */
    if (*s_pEOF)
        return 1;
