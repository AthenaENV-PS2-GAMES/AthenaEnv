...
typedef struct {
    ...
    unpack_list list;
} owl_packet __attribute__((aligned(64)));
...
static inline void owl_add_uint(owl_packet *packet, uint32_t a1) {
    uint32_t* tmp = (uint32_t*)packet->ptr;
	*tmp++ = a1;
    packet->ptr = (owl_qword *)tmp;
}

static inline void owl_add_float(owl_packet *packet, float a1) {
    float* tmp = (float*)packet->ptr;
	*tmp++ = a1;
    packet->ptr = (owl_qword *)tmp;
}

static inline void owl_add_ulong(owl_packet *packet, uint64_t a1) {
    uint64_t* tmp = (uint64_t*)packet->ptr;
	*tmp++ = a1;
    packet->ptr = (owl_qword *)tmp;
}

static inline void owl_add_tag(owl_packet *packet, uint64_t a1, uint64_t a2) {
	asm volatile ( 	
		"pcpyld $7, %1, %2 \n"
		"sq    $7,0x0(%0) \n"
...
static inline void owl_add_uquad_ptr(owl_packet *packet, __uint128_t *a1) {
	asm volatile ( 	
        "lq    $7,0x0(%1)\n" 
		"sq    $7,0x0(%0) \n"
...
static inline void owl_add_uquad(owl_packet *packet, __uint128_t a1) {
	asm volatile ( 	
        "lq    $7,0x0(%1)\n" 
		"sq    $7,0x0(%0) \n"
		 : : "r" (packet->ptr), "r" (&a1):"$7","memory");
...
static inline void owl_add_unpack_data_ref(owl_packet *packet, uint32_t t_dest_address, void *t_data, uint32_t t_size, uint8_t t_use_top) {
...
static inline void owl_add_unpack_data_cnt(owl_packet *packet, uint32_t t_dest_address, uint32_t t_size, uint8_t t_use_top) {
    owl_add_ulong(packet, DMA_TAG(t_size, 0, DMA_CNT, 0, 0, 0));
    owl_add_uint(packet, VIF_CODE(0x0101 | (0 << 8), 0, VIF_STCYCL, 0));
    owl_add_uint(packet, VIF_CODE(t_dest_address | ((uint32_t)1 << 14) | ((uint32_t)t_use_top << 15), ((t_size == 256) ? 0 : t_size), UNPACK_V4_32 | ((uint32_t)0 << 4) | 0x60, 0));
}

static inline void owl_add_unpack_data_auto(owl_packet *packet, uint32_t t_dest_address, void *t_data, uint32_t t_size, uint8_t t_use_top) {
    owl_add_ulong(packet, DMA_TAG(t_size, 0, DMA_CNT, 0, 0, 0));
    owl_add_uint(packet, VIF_CODE(0x0101 | (0 << 8), 0, VIF_STCYCL, 0));
    owl_add_uint(packet, VIF_CODE(t_dest_address | ((uint32_t)1 << 14) | ((uint32_t)t_use_top << 15), ((t_size == 256) ? 0 : t_size), UNPACK_V4_32 | ((uint32_t)0 << 4) | 0x60, 0));
}

static inline void owl_align_packet(owl_packet *packet) {
    uint32_t ptr = (uint32_t)packet->ptr;

    packet->ptr = (owl_qword*)((ptr + 15) & ~15U);
}
...
static inline void unpack_list_open(owl_packet *packet, uint32_t vu_base, bool top) {
    packet->unpack_opened = true;
    packet->list.vu_addr = vu_base;
    packet->list.top = top;
}

static inline void unpack_list_append(owl_packet *packet, void *t_data, uint32_t t_size) {
...
static inline void unpack_list_close(owl_packet *packet) {
...
static inline void owl_add_uv(owl_packet *packet, int u, int v) { // each call increases 8 bytes in pointer
	asm volatile ( 	
        "psllh $7, %[uv], 4      \n"
...
static inline void owl_add_xy(owl_packet *packet, int x, int y) { // each call increases 8 bytes in pointer
	asm volatile ( 	
        "ld      $8, %[xyoffset] \n"
        "ld      $9, %[xymax]    \n"
...
static inline void owl_add_xy_2x(owl_packet *packet, int x1, int y1, int x2, int y2) {
	asm volatile ( 	
        "lq      $8, %[xyoffset] \n"
        "lq      $9, %[xymax]    \n"
...
static inline void owl_add_xy_uv_2x(owl_packet *packet, int x1, int y1, int u1, int v1, int x2, int y2, int u2, int v2) {
	asm volatile ( 	
        "lq      $8, %[xyoffset] \n"
        "lq      $9, %[xymax]    \n"
...
static inline void owl_add_xy_uv(owl_packet *packet, int x1, int y1, int u1, int v1) {
	asm volatile ( 	
        "lq      $8, %[xyoffset] \n"
        "lq      $9, %[xymax]    \n"
...
static inline void owl_add_rgba_xy(owl_packet *packet, uint64_t rgba, int x, int y) {
	asm volatile ( 	
        "lq      $8, %[xyoffset] \n"
        "lq      $9, %[xymax]    \n"
...
static inline void owl_add_color(owl_packet *packet, uint64_t color) {
    asm volatile ( 	
...
static inline int owl_uv_transform(float fuv, int imax)
{
...
static inline int owl_coord_transform(float fxy, int offset)
{
