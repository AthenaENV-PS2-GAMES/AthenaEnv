...
static void set_tw_th_dimensions(int width, int height, int *tw, int *th)
{
    *tw = 0;
    while ((1 << *tw) < width && *tw < 31)
        (*tw)++;

    *th = 0;
    while ((1 << *th) < height && *th < 31)
        (*th)++;
}

void athena_set_tw_th(const GSSURFACE *Texture, int *tw, int *th)
{
    set_tw_th_dimensions(Texture->Width, Texture->Height, tw, th);
}
...
static uint8_t saved_depth_method[2] = { DEPTH_ALWAYS, DEPTH_ALWAYS };
...
void gs_channel_shuffle_slow(GSSURFACE *dst, uint32_t in, uint32_t out, uint32_t blockX, uint32_t blockY, GSSURFACE *source, uint32_t width, uint32_t height, GSSURFACE *pal) {
    ...
    owl_packet *packet = owl_query_packet(CHANNEL_VIF1, 204);

	owl_add_cnt_tag(packet, 203, 0);

	owl_add_uint(packet, VIF_CODE(0, 0, VIF_NOP, 0));
	owl_add_uint(packet, VIF_CODE(0, 0, VIF_NOP, 0));
	owl_add_uint(packet, VIF_CODE(0, 0, VIF_NOP, 0));
	owl_add_uint(packet, VIF_CODE(202, 0, VIF_DIRECT, 0)); 

    owl_add_tag(packet, GIF_AD, GIF_SET_TAG(5, 1, 0, 0, 0, 1));

    owl_add_tag(packet, GS_XYOFFSET_1, GS_SETREG_XYOFFSET_1(0, 0));

    set_tw_th_dimensions(width, height, &tw, &th);
...
static inline void processFrameCounter()
{
	if (frame_interval > 0 && frames >= frame_interval) {
...
int setup_buffer_textures() {
	...
	display_buffer.PageAligned = true;
...
void init_graphics() {
	if (graphics_service_initialized)
		return;
	graphics_service_initialized = true;
