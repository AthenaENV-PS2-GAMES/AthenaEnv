...
typedef struct MPEGSequenceInfo {
	...
	int m_fEOF;
	int m_MSPerFrame;
	float m_FrameRate; /* Exact frames per second; m_MSPerFrame is rounded. */
	/*
	 * Size from the sequence header. m_Width/m_Height are rounded up to whole
	 * macroblocks, which is the size of the decoded frame.
	 */
	int m_DisplayWidth;
	int m_DisplayHeight;
} MPEGSequenceInfo;
...
/* True when MPEG_Picture failed because the frame arena could not be allocated. */
extern int  MPEG_AllocFailed( void );
