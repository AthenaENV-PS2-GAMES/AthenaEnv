...
/*
 * Returns the module registered under `name`, registering it first when
 * needed. Lets several Athena modules share a driver (e.g. usbd, sio2man)
 * that the build embeds once. `dependencies` is a NULL-terminated list of at
 * most 4 names that must already be registered; returns NULL otherwise.
 */
...
#define iopman_ensure_module_buffer(name, module, dependencies, init_func, end_func) \
    iopman_ensure_module(name, module##_irx, size_##module##_irx, dependencies, init_func, end_func)
